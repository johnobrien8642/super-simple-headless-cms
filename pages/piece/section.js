...
const SectionPage = ({
	data: { section, prevSection, nextSection, writingId }
}) => {
		...
		<div className="single-section-container container my-5">
			<Head>
				...
				<meta
					name="description"
					content={`${
						pSection.piece.summary
							? pSection.piece.summary + ' By Mikowski.'
							: 'By Mikowski.'
					}`}
				/>
...
export async function getServerSideProps(context) {
	...
	const piece = await Piece.findById(section.piece).populate('sections');

	const prevSection =
		piece.sections[
			piece.sections.findIndex(
				(obj) =>
	...
	return {
		props: {
			data: {
				...
				writingId: JSON.stringify(piece._id)
			}
		}
	};
}
