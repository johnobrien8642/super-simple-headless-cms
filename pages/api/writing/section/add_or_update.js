	...
	if (req.method === 'POST') {
		...
		try {
			if (deleteBool) {
				...
				writing = await Model.findById(section[type]);

				writing.sections.splice(
					writing.sections.findIndex((_id) => _id === section._id),
					1
				);
				...
				for (let i = 0; i < writing.sections.length; i++) {
					...
					sectionUpdateObj = await Section.findById(
						sectionToUpdate._id
					);
				...
				if (update === 'true') {
					...
					writing = await Model.findById(writingId).populate(
						'sections'
					);

					writing.sections.splice(
						writing.sections.findIndex(
							(obj) =>
								obj._id.toString() === section._id.toString()
						),
						1
					);
